...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights, return_value=False):
    ...
    # N_: batch size , S_: \sum_H*W, M_ : head number, D_: feature dim of each head
    N_, S_, M_, D_ = value.shape

    # Lq_: \sum H*W, L_: multi-scale number, P_: number of sampled key points
    _, Lq_, M_, L_, P_, _ = sampling_locations.shape

    value_list = value.split(
        [H_ * W_ for H_, W_ in value_spatial_shapes], dim=1)
    # convert value from range[0,1] to [-1, 1]
